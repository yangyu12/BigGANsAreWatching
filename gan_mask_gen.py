...
class MaskGenerator(nn.Module):
    ...
    def gen_samples(self, z=None, batch_size=None):
        ...
        img = self.G(z)
        img_shifted_pos = self.G.gen_shifted(z, self.p.latent_shift_r * self.bg_direction.to(z.device))
...
def it_mask_gen(mask_gen, devices, out_device='cpu', delete_orig=True):
    ...
    while True:
        ...
        img, ref = map(torch.cat, zip(*([f.to(out_device) for f in future.result()] for future in batch_outs_future)))
        yield img, ref
